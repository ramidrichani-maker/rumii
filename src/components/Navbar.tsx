      ...
      <div className="w-full max-w-[2520px] mx-auto px-4 py-[1.58rem] bg-destructive-foreground">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => { window.location.href = '/'; }} className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 hidden md:flex items-center justify-center hover:opacity-60 transition-opacity duration-200">
            <span className="text-3xl font-title leading-none" style={{ color: '#0a0a0a', letterSpacing: '0.02em', display: 'inline-flex', alignItems: 'baseline' }}><span style={{ fontWeight: 100, display: 'inline-block', verticalAlign: 'baseline', transform: 'scaleX(0.65)', transformOrigin: 'left center' }}>r</span><span style={{ fontSize: '0.72em', display: 'inline-block', verticalAlign: 'baseline', letterSpacing: '0.03em' }}>UM</span><span style={{ fontWeight: 100, display: 'inline-block', verticalAlign: 'baseline', transform: 'scaleX(0.65)', transformOrigin: 'left center' }}>ı</span></span>
          </button>
          ...
          <div className="flex items-center gap-3 md:gap-0 md:mr-[4.5rem]">
            {/* Home icon — quick link to properties for sale */}
                ...
                >
                  <img src={houseIcon.url} alt="Properties for sale" className="h-[32px] w-[32px] md:h-[2.7rem] md:w-[2.7rem] translate-y-[0.15rem] md:translate-y-[0.3rem] object-contain" />
                </button>
              {user ? <>
                {/* My rumi (icon only) */}
                    <button
      ...
      {activeMenu && (
        ...
        >
          <div className="w-full h-full flex" style={{ transform: `translateX(-${activeMenu === 'our-world' ? 100 : activeMenu === 'services' ? 200 : 0}%)`, transition: 'transform 0.4s cubic-bezier(0.4,0,0.2,1)' }}>
            ...
            <div className="w-full h-full shrink-0 flex flex-col items-start justify-start gap-0 px-6 py-4" style={closingMenu ? { animation: 'mega-menu-content-out 0.3s ease-in both' } : { animation: 'mega-menu-content 0.4s ease-out 0.675s both' }}>
              <div className="w-1/4 ml-[17%] text-center mb-8 mt-4" style={{ animation: closingMenu ? 'none' : 'mega-menu-content 0.4s ease-out both', animationDelay: closingMenu ? '0s' : '0.71s' }}>
                <h2 className="text-5xl leading-none uppercase">
