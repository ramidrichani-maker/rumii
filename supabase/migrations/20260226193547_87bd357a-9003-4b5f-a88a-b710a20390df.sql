...
DROP POLICY IF EXISTS "Agency managers can delete their agency properties" ON properties;
DROP POLICY IF EXISTS "Agents can delete assigned properties" ON properties;
DROP POLICY IF EXISTS "Users can delete own approved properties" ON properties;

-- Recreate as PERMISSIVE (any one match = allowed)
...
ON properties FOR DELETE TO authenticated
USING (auth.uid() IN (
  SELECT property_agents.agent_id FROM property_agents
